...
workspaceSchema.virtual('admins').get(async function () {
  const memberships = await Membership.find({ workspace: this._id, role: { $in: ['admin', 'owner', 'primary_owner'] } }).populate(
    'user'
  )
...
workspaceSchema.virtual('owners').get(async function () {
  const memberships = await Membership.find({ workspace: this._id, role: { $in: ['owner', 'primary_owner'] } }).populate('user')
